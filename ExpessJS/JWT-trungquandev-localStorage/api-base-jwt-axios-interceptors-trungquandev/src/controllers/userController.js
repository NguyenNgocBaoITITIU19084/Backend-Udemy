...
const login = async (req, res) => {
  try {
    ...
    const refresh_token = JwtProvider.generateToken(payload, process.env.JWT_PUBLIC_KEY, process.env.JWT_PUBLIC_TIME)

    res.status(StatusCodes.OK).json({ message: 'Login API success!', id: MOCK_DATABASE.USER.ID, email: MOCK_DATABASE.USER.EMAIL, access_token, refresh_token })
    return
  } catch (error) {
...
const refreshToken = async (req, res) => {
  try {
    ...
    if (!refresh_token) return res.status(StatusCodes.UNAUTHORIZED).json({ message: 'No token provided' })

    const userInfor = JwtProvider.verifyToken(refresh_token, process.env.JWT_PUBLIC_KEY)
